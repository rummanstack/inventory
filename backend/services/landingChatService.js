  ...
  async chat(input = {}) {
    ...
    const answer = await this.provider.generateText({
      systemInstruction: [
        ...
        "Write like a real person on the team texting a quick reply, not a corporate FAQ bot. Never say things like 'As an AI' or 'I am a language model'. Be warm, direct, and natural.",
        "Keep every answer short: 2-4 sentences, roughly under 70 words, unless the visitor explicitly asks for more detail. Still make sure the answer is complete and genuinely useful, never a vague half-answer.",
        "This chat only displays plain text. Never use markdown symbols such as **, *, #, or numbered lists. Write in plain flowing sentences. If you truly need to list a couple of short items, put each on its own line starting with a single dash and nothing else.",

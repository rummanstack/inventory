...
export default function AuthShell({ brand, eyebrow, title, points = [], footnote, compact = false, mobilePlain = false, children }) {
  return (
    <div className={`page-shell auth-page${mobilePlain ? ' auth-page--mobile-plain' : ''}`}>
      <div className="auth-scroll relative h-screen overflow-y-auto">
        ...
        <div className="auth-ambient pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div className={`auth-stage relative flex min-h-full items-center justify-center px-0 py-0 sm:px-4 ${compact ? 'sm:py-5' : 'sm:py-8'}`}>
          <div className={`auth-frame grid w-full overflow-hidden bg-[var(--bg-dark)] shadow-[0_40px_110px_-18px_rgba(30,20,70,0.4)] ring-1 ring-[rgba(15,23,42,0.06)] sm:rounded-[28px] lg:grid-cols-[2fr_3fr] ${compact ? 'auth-frame--compact max-w-4xl' : 'max-w-5xl'}`}>

            {/* Brand panel — full hero on desktop, compact header bar on mobile */}
            <div className={`auth-brand-panel relative flex flex-col justify-between overflow-hidden p-6 pb-8 ${compact ? 'lg:p-8' : 'lg:p-10'}`}>
              ...
              <div className="auth-brand-content relative">
                <Link to="/" className="auth-logo-link inline-flex items-center gap-3" aria-label="StockLedger">
                  <span className="auth-logo-chip logo-chip flex h-11 w-11 items-center justify-center rounded-2xl shadow-[0_10px_24px_rgba(0,0,0,0.35)]">
                    <img src={stockLedgerLogoIcon} alt="" className="h-9 w-9 object-contain" />
                  </span>
                  <span>
                    <span className="auth-logo-name block text-lg font-black leading-none tracking-tight text-white">StockLedger</span>
                    <span className="auth-logo-subtitle mt-1 block text-[10px] font-bold uppercase tracking-[0.2em] text-[#e9e2ff]/75">{brand}</span>
                  </span>
                </Link>

                <p className={`auth-brand-eyebrow mt-8 flex w-fit items-center gap-2 rounded-full border border-[rgba(255,255,255,0.16)] bg-[rgba(255,255,255,0.07)] px-3.5 py-1.5 text-[10px] font-black uppercase tracking-[0.22em] text-[#ddd2ff] backdrop-blur ${compact ? 'lg:mt-8' : 'lg:mt-12'}`}>
                  <span className="h-1.5 w-1.5 rounded-full bg-[#2dd4bf] shadow-[0_0_0_3px_rgba(45,212,191,0.18)]" />
                  {eyebrow}
                </p>
                {title ? (
                  <h2 className="auth-brand-title mt-3 max-w-xs text-xl font-black leading-[1.2] tracking-tight text-white lg:mt-4 lg:text-[28px] lg:leading-[1.15]">
                    {title}
                  </h2>
                ) : null}
              </div>
            ...
            {/* Form panel */}
            <div className={`auth-form-panel flex flex-col justify-center rounded-t-[28px] bg-white px-6 py-10 sm:px-12 lg:rounded-none ${compact ? 'sm:py-9' : 'sm:py-12'}`}>
              {children}
            </div>

                    ...
                    <div className={isElectronics ? 'grid gap-3 lg:grid-cols-[minmax(0,1.8fr)_minmax(110px,0.6fr)_minmax(110px,0.6fr)_minmax(110px,0.6fr)_auto]' : 'grid gap-3 lg:grid-cols-[minmax(0,1.8fr)_minmax(110px,0.6fr)_minmax(110px,0.6fr)_minmax(110px,0.6fr)_minmax(110px,0.6fr)_auto]'}>
                      ...
                      {!isElectronics ? (
                        <div>
                          ...
                          <input className="input" type="number" inputMode="decimal" min="0" value={row.caseQty} onChange={(event) => vm.updateItem(row.rowId, 'caseQty', event.target.value)} />
                        </div>
                      ) : null}
                      <div>
                        <label className="label">{t('common.piece')}</label>
                        <input className="input" type="number" inputMode="decimal" min="0" value={row.pieceQty} onChange={(event) => vm.updateItem(row.rowId, 'pieceQty', event.target.value)} />
                      </div>
                      <div>
                        <label className="label">{t('purchaseReceive.purchasePriceLabel')}</label>
                        <input className="input" type="number" inputMode="decimal" min="0" step="0.0001" value={row.purchasePrice} onChange={(event) => vm.updateItem(row.rowId, 'purchasePrice', event.target.value)} />
                      </div>
                      <div>
                        <label className="label">{t('purchaseReceive.lineDiscountLabel')}</label>
                        <input className="input" type="number" inputMode="decimal" min="0" step="0.0001" value={row.lineDiscount} onChange={(event) => vm.updateItem(row.rowId, 'lineDiscount', event.target.value)} />
                      </div>
            ...
            <dl className="mt-3 space-y-2 text-sm">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <dd className="flex items-center gap-2">
                  <input className="input h-9 w-28 text-right" type="number" inputMode="decimal" min="0" step="0.0001" value={vm.discountInput} onChange={(event) => vm.setDiscountInput(event.target.value)} disabled={saving} />
                </dd>
              </div>
              {vm.taxRate > 0 ? (
                <>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <dd className="flex items-center gap-2">
                  <input className="input h-9 w-28 text-right" type="number" inputMode="decimal" min="0" step="0.0001" value={vm.paidAmountInput} onChange={(event) => vm.setPaidAmountInput(event.target.value)} disabled={saving} />
                </dd>
              </div>

...
const PURCHASE_RECEIVE_SHORTCUTS = {
...
export default function PurchaseReceivePage() {
  ...
  async function handleDownloadPdf() {
    await downloadPdf(async () => {
      await inventoryApi.recordPrint({ entityType: 'purchase_receive_list', entityId: null, label: 'pdf' }).catch(() => {});
      await downloadSheetPdf(PURCHASE_RECEIVE_PRINT_ID, 'purchase-receive.pdf');
    });
  }

  function handlePrint() {
    inventoryApi.recordPrint({ entityType: 'purchase_receive_list', entityId: null, label: 'print' }).catch(() => {});
    printElementById(PURCHASE_RECEIVE_PRINT_ID);
  }
  ...
  useEffect(() => {
    function handleKeyDown(event) {
      if (matchesShortcut(event, PURCHASE_RECEIVE_SHORTCUTS.add) && canManagePurchases && !formModal && !viewReceipt) {
        event.preventDefault();
        setFormModal({ mode: 'add' });
      } else if (matchesShortcut(event, PURCHASE_RECEIVE_SHORTCUTS.pdf) && !downloadingPdf) {
        event.preventDefault();
        handleDownloadPdf();
      } else if (matchesShortcut(event, PURCHASE_RECEIVE_SHORTCUTS.excel)) {
        event.preventDefault();
        handleExportExcel();
      } else if (matchesShortcut(event, PURCHASE_RECEIVE_SHORTCUTS.print)) {
        event.preventDefault();
        handlePrint();
      }
    }
  ...
  }, [downloadingPdf, canManagePurchases, formModal, viewReceipt, vm.purchaseNumber, vm.supplierId, vm.supplierInvoiceNo, vm.dateFrom, vm.dateTo, vm.paymentStatus, t]);

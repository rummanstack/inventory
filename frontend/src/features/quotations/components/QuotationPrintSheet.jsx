    ...
    <div id={targetId} className={cx('mx-auto w-full max-w-[210mm] bg-white p-8 font-sans text-slate-950', printTarget && 'print-target')}>
      ...
      <div className="flex items-start justify-between pb-5 border-b-2 border-slate-950">
        ...
        <div className="text-right">
          <p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-400">Quotation</p>
          <p className="mt-1 text-2xl font-black text-slate-950">{quotation?.quoteNumber}</p>
          <div className="mt-2 space-y-0.5 text-xs text-slate-500">
            <p>Date: <span className="font-semibold text-slate-800">{formatDate(quotation?.quoteDate, language)}</span></p>
            {quotation?.validUntil ? (
              <p>Valid until: <span className="font-semibold text-slate-800">{formatDate(quotation.validUntil, language)}</span></p>
            ) : null}
          </div>
        </div>
      </div>
      ...
      {(quotation?.customerName || quotation?.customerPhone || quotation?.customerEmail) ? (
        <div className="mt-5 grid grid-cols-2 gap-6">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Prepared For</p>
      ...
      <table className="mt-6 w-full border-collapse text-[10px]">
        <thead>
          <tr className="bg-slate-950 text-white">
            ...
            <th className="border border-slate-800 px-2 py-2 text-left font-bold">Product / Description</th>
            <th className="border border-slate-800 px-2 py-2 text-right font-bold">Qty</th>
            <th className="border border-slate-800 px-2 py-2 text-right font-bold">Unit Price</th>
            <th className="border border-slate-800 px-2 py-2 text-right font-bold">Discount</th>
            <th className="border border-slate-800 px-2 py-2 text-right font-bold">Total</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item, i) => (
      ...
      <div className="mt-4 flex justify-end">
        <div className="w-60 space-y-1.5 text-[11px]">
          ...
          {taxAmount > 0 ? (
            <div className="flex justify-between">
              <span className="text-slate-500">Tax ({quotation?.taxRate}%)</span>
      ...
      {quotation?.notes ? (
        <div className="mt-6 rounded border border-amber-200 bg-amber-50 px-4 py-3">
          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-amber-600">Notes</p>
      ...
      <div className="mt-6 border-t border-slate-200 pt-4 text-[10px] text-slate-500">
        <p className="font-bold uppercase tracking-[0.14em]">Terms &amp; Conditions</p>
        <p className="mt-1">
          {quotation?.validUntil
            ? `This quotation is valid until ${formatDate(quotation.validUntil, language)}. `
            : ''}
          Prices are subject to change after the validity period. All amounts are inclusive of applicable taxes as shown.
        </p>
      </div>
      ...
      <div className="mt-10 flex justify-between text-[11px] font-semibold text-slate-950">
        <div className="w-40">
          <div className="border-t border-slate-900 pt-2">Customer Acceptance</div>
        </div>
        <div className="w-40">
      ...
      <div className="mt-8 border-t border-slate-200 pt-3 text-center text-[9px] text-slate-400">
        ...
        <p className="mt-0.5">Thank you for considering us — we look forward to working with you.</p>
      </div>
    </div>
  );
}

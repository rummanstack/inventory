    ...
    <div>
      ...
      <div id={SALES_INVOICES_PRINT_ID} className="surface overflow-hidden print-target">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:flex-wrap sm:items-center">
          ...
          <Select className="input w-full sm:w-44" value={vm.customerId} onChange={(event) => vm.setCustomerId(event.target.value)}>
            <option value="">{t('retailer.shared.allCustomers')}</option>
            {retailCustomerDirectory.map((customer) => (
              <option key={customer.id} value={customer.id}>{customer.name}</option>
            ))}
          </Select>
          <Select className="input w-full sm:w-40" value={vm.saleType} onChange={(event) => vm.setSaleType(event.target.value)}>
          ...
          <Select className="input w-full sm:w-40" value={vm.paymentStatus} onChange={(event) => vm.setPaymentStatus(event.target.value)}>
